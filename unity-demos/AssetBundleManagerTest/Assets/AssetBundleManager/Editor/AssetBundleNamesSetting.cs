...
namespace AssetBundles
{
    ...
    public struct AssetBundlePathSetting
    {
        ...
        public void SetName()
        {
            ...
            {
                if (!AssetBundleNamesSetting.SetBundleName(path, name, variant, string.IsNullOrEmpty(ignore) ? null : ignore.Split(','), separated, separetedSubFiler, out errMsg))
                {
                    success = false;
                }
            }
    ...
    public class AssetBundleNamesSetting : ScriptableObject
    {
        ...
        [DictionaryDrawerSettings(KeyLabel = "name",ValueLabel = "111")]
        [ShowInInspector]
        ...
        public void SetBundleNames()
        {
            ...
            foreach (AssetBundlePathSetting setting in list)
            {
                ...
                {
                    if (!SetBundleName(setting.path, setting.name, setting.variant, string.IsNullOrEmpty(setting.ignore) ? null : setting.ignore.Split(','), setting.separated, setting.separetedSubFiler, out errMsg))
                    {
                        success = false;
                        break;
                    }
        ...
        public static bool SetBundleName(string directory, string name, string variant, string[] ignores, bool separateFile, bool separateSubFolder, out string err)
        {
            err = string.Empty;
